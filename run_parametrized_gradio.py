...
# -*- coding: utf-8 -*-
"""
Run script for the Parametrized 3WayCoT Framework

This script completes and runs the parametrized Gradio interface with all necessary
functions to handle use case selection, parameter customization, and enhanced visualizations.
"""
...
# Import the parametrized Gradio app
from parametrized_gradio_app import (
    ParametrizedGradio3WayCoT, 
...
logger = logging.getLogger('3WayCoT.ParametrizedInterface')

def create_reasoning_steps_table(reasoning_steps: List[Dict]):
    ...
    if not reasoning_steps or not isinstance(reasoning_steps, list):
        return pd.DataFrame(columns=['Step', 'Reasoning', 'Confidence', 'Metrics'])
    ...
    for i, step in enumerate(reasoning_steps):
        ...
        confidence_level = step.get('confidence_level', '')
        confidence_method = step.get('confidence_method', '')
        assumptions = step.get('assumptions', [])
        
        # Format the metrics as a string
        metrics_str = ", ".join([f"{k}: {v:.2f}" for k, v in metrics.items() if isinstance(v, (int, float))]) if metrics else ""
        
        # Format assumptions as a string
        assumptions_str = "\n".join(assumptions) if assumptions else ""
        
        # Create a row for the table
        row = {
            'Step': f"Step {step_num}{' (Final)' if is_final else ''}",
            'Reasoning': reasoning[:200] + "..." if len(reasoning) > 200 else reasoning,
            'Confidence': f"{confidence:.2f} ({confidence_level})" if confidence_level else f"{confidence:.2f}",
            'Method': confidence_method[:50] + "..." if len(confidence_method or '') > 50 else confidence_method,
            'Metrics': metrics_str,
            'Assumptions': assumptions_str[:100] + "..." if len(assumptions_str) > 100 else assumptions_str,
        }
...
def create_interface():
    ...
    with gr.Blocks(title="Parametrized 3WayCoT Framework", theme=gr.themes.Soft()) as interface:
        ...
        Analyze prompts using Three-Way Decisions with customizable parameters and confidence metrics visualization.
        """)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Group():
                    ...
                    # Dataset section
                    with gr.Accordion("Dataset Processing", open=False):
                        dataset_keys = list(app.datasets.keys())
                        ...
                        """ if not dataset_keys else f"""**{dataset_keys[0]}**: {app.datasets[dataset_keys[0]].get('items', 0)} items""")
            ...
            with gr.Column(scale=2):
                ...
                with gr.Tabs(selected=0):
                    ...
                    with gr.TabItem("Reasoning Steps"):
                        reasoning_table = gr.DataFrame(label="Reasoning Steps")
                    
                    with gr.TabItem("Raw Output"):
                        raw_output = gr.JSON(label="Raw Results")
                    
                    with gr.TabItem("History"):
                        history_table = gr.DataFrame(label="History", headers=["Timestamp", "Prompt", "Result"], interactive=False)
        ...
        def update_dataset_info(dataset_key):
            ...
            if dataset_key and dataset_key in app.datasets:
                dataset = app.datasets[dataset_key]
                return f"**{dataset_key}**: {dataset.get('items', 0)} items"
        ...
        def process_and_display(prompt, context, alpha, beta, tau, confidence_weight, temp, max_tokens, top_p, model):
            ...
            try:
                ...
                steps_table = create_reasoning_steps_table(reasoning_steps)
        ...
        def clear_inputs():
            ...
            return (
                "", "",  # prompt, context
                "", "", "", "", "", "",  # outputs
                None, None, None, None, None  # visualizations
            )
        
        def load_history():
            """Load and display the history table."""
            history_data = []
            for item in app.history:
                try:
                    timestamp = item.get('timestamp', '')
                    prompt = item.get('prompt', '')
                    result_summary = "Success" if item.get('result', {}).get('status', '') != 'error' else "Error"
                    history_data.append([timestamp, prompt, result_summary])
                except Exception as e:
                    logger.error(f"Error formatting history item: {e}")
            
            return pd.DataFrame(history_data, columns=["Timestamp", "Prompt", "Result"])
        ...
        # Add examples focused on confidence and decision making
        gr.Examples(
            examples=[
